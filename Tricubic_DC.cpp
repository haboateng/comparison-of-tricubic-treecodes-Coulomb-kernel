// Tricubic discontinuous version with or without finite differences
    ...
    if (tree[panel_index].MAC < R_sq)
      {
        ...
        Compute_Coeff_B2(p_x, p_y, p_z, panel_index, kernel);
			...
			const Kernel& kernel)
{
    ...
    if (tree[panel_index].MAC < R_sq)
      {
        vec_4d tree_result = Call_Tricubic_B2(p_x, p_y, p_z,
					      panel_index,
					      kernel);
...
*/
    ...
    //***************** Compute moment for each panel **************
	   ...
	   Panel_Moment_Tricubic(k, lambda, particles, tm, tmx, tmy, tmz);
           BinvMultiply_Transpose_B2(tm, tree[k].moments);
           BinvMultiply_Transpose_B2(tmx, tree[k].dxmoments);
           BinvMultiply_Transpose_B2(tmy, tree[k].dymoments);
           BinvMultiply_Transpose_B2(tmz, tree[k].dzmoments);
         }
