// Tricubic C-0 version with or without finite differences
...
// Multiply coefficient by B inverse
   ...
   BinvMultiply_B6(tree[panel_index].moments, tm);
    ...
    if (tree[panel_index].MAC < R_sq)
      {
        ...
        Compute_Coeff_B6(p_x, p_y, p_z, panel_index, kernel);
			...
			const Kernel& kernel)
{
    ...
    if (tree[panel_index].MAC < R_sq)
      {
        vec_4d tree_result = Call_Tricubic_B6(p_x, p_y, p_z,
					      panel_index,
					      kernel);
...
/*
    // Read the inverse matrix from a file
       ...
       ifstream BinvFile("Bnew6InvMatrix.txt");
       for (int i=0; i<64; i++)
         {
           for (int j=0; j<64; j++)
             {
...
*/
    ...
    //***************** Compute moment for each panel **************
	   ...
	   Panel_Moment_Tricubic(k, lambda, particles, tm, tmx, tmy, tmz);
           BinvMultiply_Transpose_B6(tm, tree[k].moments);

//=========================================================
/*
       for (int i=0; i<64; i++)
        {
          tree[k].moments[i] = 0.0;
          for (int j=0; j<64; j++)
            {
              tree[k].moments[i] += Binv[j][i]*tm[j];
            }
        }
*/
//=========================================================
           BinvMultiply_Transpose_B6(tmx, tree[k].dxmoments);
           BinvMultiply_Transpose_B6(tmy, tree[k].dymoments);
           BinvMultiply_Transpose_B6(tmz, tree[k].dzmoments);
         }
...
// Sum up forces
  ...
  while (true)
    ...
    cout << " " << endl;
    cout << "max radius = " <<" " << maxCradius << endl;
    cout << "R = " << " " << theta << endl;
// Output to file
    ...
    << E <<"  "<< FE << "  "<< eL1 <<"  "<< fL1 <<"  "<<ds_cpu_time <<"  "<<treecode_cpu_time << " " << maxCradius/theta 
    <<" "<< maxCradius/(maxCradius + theta) << endl;
